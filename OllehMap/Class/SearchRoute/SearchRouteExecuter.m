...
- (void) searchRoute_Car:(int)optionType
{
    ...
    if (oms.searchResultRouteStart.isCurrentLocation)
    {
        [[ServerConnector sharedServerConnection] requestGeocodingCoordForSearchRoute:self action:@selector(didRefineTargetAddress:) type:1 x:oms.searchResultRouteStart.coordLocationPoint.x y:oms.searchResultRouteStart.coordLocationPoint.y dong:1 searchType:optionType];
    }
    else if ( oms.searchResultRouteVisit.used && oms.searchResultRouteVisit.isCurrentLocation)
    {
        [[ServerConnector sharedServerConnection] requestGeocodingCoordForSearchRoute:self action:@selector(didRefineTargetAddress:) type:2 x:oms.searchResultRouteVisit.coordLocationPoint.x y:oms.searchResultRouteVisit.coordLocationPoint.y dong:1 searchType:optionType];
    }
    else if (oms.searchResultRouteDest.isCurrentLocation)
    {
        [[ServerConnector sharedServerConnection] requestGeocodingCoordForSearchRoute:self action:@selector(didRefineTargetAddress:) type:3 x:oms.searchResultRouteDest.coordLocationPoint.x y:oms.searchResultRouteDest.coordLocationPoint.y dong:1 searchType:optionType];
    }
    // 바로 경로탐색 시작해도 됨.
    else
    {
...
- (void) didFinish_SearchRoute_Car :(id)request
{
    ...
    if ([request finishCode] == OMSRFinishCode_Completed)
    {
        ...
        if(oms.searchRouteData.isRouteCar)
        {
            ...
            // 길찾기 화면 띄우자
            SearchRouteResultMapViewController *srrmvc = [board instantiateViewControllerWithIdentifier:@"SearchRouteResultMapView"];
            [srrmvc setCurrentRouteCarSelector:routeOptionType];
...
- (void) searchRoute_Public:(int)priority pubicSearchIndex:(int)index
{
    OllehMapStatus *oms = [OllehMapStatus sharedOllehMapStatus];
    ...
    else
    {
        ...
        pubRouteOptionTye = priority;
        publicMapIndex = index;
        NSLog(@"publicMapIndex : %d", publicMapIndex);
        NSLog(@"priority : %d", priority);
        
        [[ServerConnector sharedServerConnection] requestRouteSearch:self action:@selector(didFinish_SearchRoute_Public:) SX:oms.searchResultRouteStart.coordLocationPoint .x SY:oms.searchResultRouteStart.coordLocationPoint .y EX:oms.searchResultRouteDest.coordLocationPoint.x EY:oms.searchResultRouteDest.coordLocationPoint.y RPType:1/*자동차검색*/ CoordType:7 VX1:oms.searchResultRouteVisit.coordLocationPoint.x VY1:oms.searchResultRouteVisit.coordLocationPoint.y Priority:priority];
    }

}
- (void) searchRoute_Public:(int)priority
{
    OllehMapStatus *oms = [OllehMapStatus sharedOllehMapStatus];
...
- (void) didFinish_SearchRoute_Public :(ServerRequester*)request
{
    ...
    if ([request finishCode] == OMSRFinishCode_Completed)
    {
        ...
        if(oms.searchRouteData.isRoutePublic)
        {
            ...
            if(!publicMapIndex == 0)
            {
            switch (pubRouteOptionTye)
            {
                case OM_SRRMV_PublicMethodType_Recommend:
                    if (oms.searchRouteData.routePublicRecommendCount > publicMapIndex-1)
                        routeDic = [oms.searchRouteData.routePublicRecommend objectAtIndexGC:publicMapIndex-1];
                    break;
                case OM_SRRMV_PublicMethodType_Bus:
                    if (oms.searchRouteData.routePublicBusCount > publicMapIndex-1)
                        routeDic = [oms.searchRouteData.routePublicBus objectAtIndexGC:publicMapIndex-1];
                    break;
                case OM_SRRMV_PublicMethodType_Subway:
                    if (oms.searchRouteData.routePublicSubwayCount > publicMapIndex-1)
                        routeDic = [oms.searchRouteData.routePublicSubway objectAtIndexGC:publicMapIndex-1];
                    break;
                case OM_SRRMV_PublicMethodType_Both:
                    if (oms.searchRouteData.routePublicBothCount > publicMapIndex-1)
                        routeDic = [oms.searchRouteData.routePublicBoth objectAtIndexGC:publicMapIndex-1];
                    break;
                default:
                    routeDic = nil;
                    break;
            ...
            UIStoryboard *board = [UIStoryboard storyboardWithName:@"MainStoryboard" bundle:nil];
            SearchRouteResultMapViewController *srrmvc = [board instantiateViewControllerWithIdentifier:@"SearchRouteResultMapView"];
            [srrmvc setCurrentPublicMethod:pubRouteOptionTye];
            srrmvc.isPublicFirst = YES;
            srrmvc.publicIndex = publicMapIndex;
