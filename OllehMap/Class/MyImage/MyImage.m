    ...
    if (userMyImage)
    {
        ...
        switch (myImageIndex)
        {
            ...
            case 4:
            {
                ...
                // 기존이미지
                UIImage *oldMyImage = [UIImage imageWithContentsOfFile:documentImageFilePath];
                // 배경이미지(흰색고정)
                UIImage *backgroundMyImage = [UIImage imageNamed:@"modifyed.png"];
                ...
                CGSize newImageSize = CGSizeMake(39 * reti, 43 * reti);
                UIGraphicsBeginImageContext( newImageSize );
                // Use existing opacity as is
                [backgroundMyImage drawInRect:CGRectMake(0,0,newImageSize.width,newImageSize.height)];
                
                // Apply supplied opacity if applicable
                [oldMyImage drawInRect:CGRectMake(0,0,newImageSize.width,newImageSize.height) blendMode:kCGBlendModeNormal alpha:1];
                
                UIImage *flippedImage = UIGraphicsGetImageFromCurrentImageContext();
                UIGraphicsEndImageContext();
                
                NSString *modiImgPath = [NSString stringWithFormat:@"%@/MyImage%@.PNG", [documentsDirecotryPathArray objectAtIndexGC:0], isRetina ? @"@2x" : @""];
                NSData *mskedImageData = [NSData dataWithData:UIImagePNGRepresentation(flippedImage)];
                [mskedImageData writeToFile:modiImgPath atomically:YES];
