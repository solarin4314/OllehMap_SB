...
{
    // 네비게이션 뷰 생성
    _vwNavigation = [[UIView alloc] initWithFrame:CGRectMake(0, OM_STARTY, 320, 37)];
    [self.view addSubview:_vwNavigation];
    
    // 즐겨찾기 컨테이너 초기화
    _vwFavoriteContainer = [[UIView alloc]
                            initWithFrame:CGRectMake(0, OM_STARTY + 37,
...
- (void) renderCategory :(Favorite_Category)category
{
    ...
    for (int i=0, maxi=(int)[(NSArray *)categories count]; i<maxi; i++)
    {
...
- (void) renderCategoryTable
{
    ...
    if (_tvwFavoriteList.isEditing)
    {
        ...
        rectFavoriteList.size.height = tableHeight-37-57-(OM_STARTY + 37);
    }
    else
    {   // 전체높이-네비게이션높이-카테고리높이
        rectFavoriteList.size.height = tableHeight-(37 + OM_STARTY)-36;
    }
...
- (void) tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath
{
    ...
    for (int i=0,maxi=(int)_favoriteAllList.count; i<maxi; i++)
    {
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //BOOL isSettingFavorite = [vc isKindOfClass:[SettingViewController class]];
    ...
    NSString *rdCd = stringValueOfDictionary(favoriteDic, @"RdCd");
    ...
    if (category == Favorite_Category_Local || category == Favorite_Category_Public)
    {
        ...
        else
        {
            ...
            else if (![rdCd isEqualToString:@""])
            {
                [[ServerConnector sharedServerConnection] requestRoadNameSearch:self action:@selector(SearchfinishedRoadSearch:) roadCode:rdCd];
            }
            else
            {
//                // 무조건 지도보기
...
//                main.nMapRenderSinglePOICategory = MainMap_SinglePOI_Type_Favorite;
//                [[OMNavigationController sharedNavigationController] pushViewController:main animated:YES];
                
                // 클래스메서드 적용
                [MainViewController markingSinglePOI_RenderType:MapRenderType_SearchResult_SinglePOI category:MainMap_SinglePOI_Type_Favorite animated:YES];
...
- (UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    FavoriteCell *cell = [[FavoriteCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
...
- (void) didFinishRequestBusNumberDetail :(id)request
{
	...
	{
        BusLineViewController *bnvc = [self.storyboard instantiateViewControllerWithIdentifier:@"BusLineView"];
        [[OMNavigationController sharedNavigationController] pushViewController:bnvc animated:YES];
    }
...
- (void) onRenameFavorite:(id)sender
{
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"즐겨찾기 이름" message:@"" delegate:self cancelButtonTitle:@"취소" otherButtonTitles:@"확인", nil];
    alert.alertViewStyle = UIAlertViewStylePlainTextInput;
    
    UITextField *textField = [alert textFieldAtIndex:0];
    [textField setFont:[UIFont systemFontOfSize:16]];
    [textField setText:rename];
    ...
    if(buttonIndex == 1)
    {
        ...
        for (NSMutableDictionary *dic in _favoriteAllList)
        {
            ...
            if([checkNum boolValue])
            {
                ...
                [dic setObject:textField.text forKey:@"Title1"];
                break;
            }
...
- (void) SearchfinishedRoadSearch:(id)request
{
    if([request finishCode] == OMSRFinishCode_Completed)
    {
