...
- (void) setSelected:(BOOL)selected
{
    ...
    if (selected)
    {
        ...
        {
            if(single_coordY == 0)
                single_coordY = marker.frame.origin.y;
            
            [marker setFrame:CGRectMake(marker.frame.origin.x, single_coordY - 2, marker.frame.size.width, marker.frame.size.height)];
        }
    ...
    else
    {
        ...
        for (UIImageView *marker in self.getOverlayView.subviews)
        {
            ...
            [marker setFrame:CGRectMake(marker.frame.origin.x, single_coordY, marker.frame.size.width, marker.frame.size.height)];
        }
    }
...
// 검색결과 라인POI를 처리하기위한 오버레이 클래스를 정의한다.
@implementation OMImageOverlayLineSingle
@synthesize lineUsePOIIcon = _lineUsePOIIcon;
- (void) initComponentsSub
{
    _lineUsePOIIcon = NO;
    _lineCoordY = 0;
...
- (void) setSelected:(BOOL)selected
{
    ...
    if (selected)
    {
        ...
        {
            if(_lineCoordY == 0)
                _lineCoordY = marker.frame.origin.y;
    ...
    NSString *imageName = [NSString stringWithFormat:@"line_map_b_marker%@%@.png", imageName_POI, imageName_Selected];
    UIImage *poiIconImage = [UIImage imageNamed:imageName];
    [self.imageView setImage:poiIconImage];
    [self setCenterOffset:CGPointMake(self.imageSize.width/2, self.imageSize.height)];
}
...
- (void) setSelected:(BOOL)selected
{
    ...
    if (selected)
    {
        ...
        NSLog(@"subView count : %d", (int)self.getOverlayView.subviews.count);
        
        for (UIImageView *marker in self.getOverlayView.subviews)
        {
            if(multi_coordY == 0)
                multi_coordY = marker.frame.origin.y;
            
            [marker setFrame:CGRectMake(marker.frame.origin.x, multi_coordY - 2, marker.frame.size.width, marker.frame.size.height)];
        }
    }
    else
    {
        ...
        if([[self.additionalInfo objectForKeyGC:@"Index"] intValue] == indexer)
        {
            for (UIImageView *marker in self.getOverlayView.subviews)
            {
                [marker setFrame:CGRectMake(marker.frame.origin.x, multi_coordY, marker.frame.size.width, marker.frame.size.height)];
            }
        }
...
//    if(_duplicated)
//    {
//        NSString *imagePathSingle = [[NSBundle mainBundle] pathForResource:@"map_b_marker_overlap.png" ofType:nil];
//        themeImage =[UIImage imageWithContentsOfFile:imagePathSingle];
//    }
//    else
//    {
