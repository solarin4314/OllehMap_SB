...
- (void)layoutFolderView:(CGRect)selectedRect
{
    ...
    folderViewFrame.origin.y = selectedRect.origin.y + selectedRect.size.height + 20 + 37 + OM_STARTY;
...
- (void) initer
{
    ...
    //    _folderView = [[UIView alloc] initWithFrame:CGRectMake(0, 200, 320, 189)];
    //    [_folderView setBackgroundColor:[UIColor clearColor]];
    //
    //
    //    _folderBg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"theme_list_01.png"]];
    //    [_folderBg setFrame:CGRectMake(0, 0, 320, 189)];
    //    [_folderView addSubview:_folderBg];
    //
    //    _shadowTopBg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"shadow_01.png"]];
    //    [_shadowTopBg setFrame:CGRectMake(0, 0, 320, 15)];
    //    [_folderView addSubview:_shadowTopBg];
    //
    //    _shadowBottomBg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"shadow_02.png"]];
    //    [_shadowBottomBg setFrame:CGRectMake(0, 174, 320, 15)];
    //    [_folderView addSubview:_shadowBottomBg];
    //
    //    _elseView = [[UIControl alloc] initWithFrame:CGRectMake(0, 0, 320, self.view.frame.size.height)];
    //
    //    _elseViewBg = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 320, _elseView.frame.size.height)];
    //    [_elseView addSubview:_elseViewBg];
    //
    //    [self.view addSubview:_folderView];
    //    [self.view addSubview:_elseView];
    //
    //    [_folderView setHidden:YES];
    //    [_elseView setHidden:YES];
}
- (void) drawTheme
{
...
- (void) drawBatch
{
    ...
    for (int i=0; i<themeCount; i++)
    {
        ...
        if ([self timeInterVal:updateDateStr] < 7)
        {
            ...
            // 왜 12만큼 줄어야될까?
            [itemNew setFrame:CGRectMake(6 + new_x, 3 + new_y, 24, 24)];
        ...
        [blurView setFrame:CGRectMake(6+new_x, new_y, 67+10, 67+30)];
...
- (void) themeClick:(id)sender
{
    ...
    themeDetailCount = [(NSMutableDictionary *)[themeInfo objectForKeyGC:@"sub"] count];
    
    // 선택한 테마의 좌표값
    CGRect sRect = [[[_btnCrdDictionary objectForKeyGC:[NSString stringWithFormat:@"%d", index]] objectAtIndexGC:0] CGRectValue];
    ...
    if (_folderView.hidden) // 만약 폴더가 열려 있지 않으면...
    {
        ...
        CGRect elseFrame = CGRectMake(0, 37 + OM_STARTY + selectedRect.origin.y+selectedRect.size.height + 20, 320, self.view.frame.size.height - OM_STARTY - 37);
...
-(void) subThemeClick:(id)sender
{
    ...
    NSString *themeName = oms.themeInfoList [index] [@"name"];
    
    // MIK.geun :: 20120929
    // maxRenderingZoomLevel 은 서버에서 받도록 수정~
    // themeCode 별도로 상위 코드인 mainThemeCode 도필요 (아이콘용.)
    [MainViewController markingThemePOI_ThemeCode:themeCode mainThemeCode:oms.themeInfoList [index] [@"code"] maxRenderingZoomLevel:[oms.themeInfoList [index] [@"scale"] intValue] selectedThemeName:themeName animated:NO];
...
{
    CGRect elseFrame = CGRectMake(0, 37 + OM_STARTY + rect.origin.y+rect.size.height + 20, 320,
