...
- (void) removePolygonOverlay
{
    for (Overlay *currentOverlay in self.getOverlays)
    {
        if([currentOverlay isKindOfClass:[OMPolylineOverlay class]])
            [self removeOverlay:currentOverlay];
    }
}
...
- (void) showMapContainer:(UIView *)sView :(id)delegate
{
    ...
    NSLog(@"sView frame : %@", NSStringFromCGRect(sView.frame));
    ...
    // 5.0 문제
    //[_kmap setFrame:CGRectMake(0, OM_STARTY, 320, [UIScreen mainScreen].bounds.size.height - OM_STARTY)];
    
    // 변경된 해상도 맵이 삽입되도록 처리하자. (특히 길찾기 모드에서 죽지 않기위해서..)
    [self.kmap setMapDisplay:[[OllehMapStatus sharedOllehMapStatus] getDisplayMapResolution]];
    
    NSLog(@"kmap frame : %@", NSStringFromCGRect(_kmap.frame));
