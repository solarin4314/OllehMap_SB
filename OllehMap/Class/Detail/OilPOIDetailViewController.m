...
- (void) initer
{
    _underline = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 1)];
    UIImageView *line = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"poi_list_line_03.png"]];
    [_underline addSubview:line];
...
- (void) drawTopView
{
    ...
    // 업종명
    UILabel *classify = [[UILabel alloc] initWithFrame:CGRectMake(90, segmentHeight, 220, 13)];
    ...
    [topView setFrame:CGRectMake(X_VALUE, _oilViewStartY, X_WIDTH, topViewHeight)];
    // 스크롤뷰에 탑뷰 넣기
...
-(void)finishRequestoilDetailAtPoiId2:(id)request;
{
    if ([request finishCode] == OMSRFinishCode_Completed)
    {
        ...
        else
        {
            ...
            if(_oilViewStartY + 37 <= self.view.frame.size.height)
            {
                _oilViewStartY = self.view.frame.size.height - 37;
            }
...
- (void) nullData
{
    ...
    int oilInfoNullViewHeight = self.view.frame.size.height - (37 + OM_STARTY + 37) - _oilViewStartY;
    // 유가정보리스트없는 뷰
    ...
    [nullLabel setText:NSLocalizedString(@"Msg_Detail_InfoNull", @"")];
    [nullLabel setTextAlignment:NSTextAlignmentCenter];
    [nullLabel setFont:[UIFont systemFontOfSize:13]];
    [nullLabel setTextColor:convertHexToDecimalRGBA(@"8b", @"8b", @"8b", 1.0)];
...
- (void) drawButtonView
{
    ...
    [_bottomView setFrame:CGRectMake(0, _oilViewStartY, 320, 37)];
