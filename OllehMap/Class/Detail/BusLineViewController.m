...
-(void) viewWillAppear:(BOOL)animated
{
    ...
    [_busNumberWindow setFrame:CGRectMake(0, OM_STARTY + 37, 320, self.view.frame.size.height - (37 + OM_STARTY))];
...
- (void) drawingTop
{
    ...
    [topView setFrame:CGRectMake(0, _viewStartY, 320, 90)];
...
- (void) drawingRealTime
{
    ...
    for (int i=0; i<busStateArray.count; i++)
    {
        ...
        [busNumberImg setImage:[UIImage imageNamed:@"bus_icon_s.png"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([stId isEqualToString:@""] && [stationNo isEqualToString:@""])
    {
        [OMMessageBox showAlertMessage:@"" :NSLocalizedString(@"Msg_BusNumber_NotSTID", @"")];
    }
    else if([stId isEqualToString:@""])
    {
...
{
    return [(NSMutableDictionary *)[[OllehMapStatus sharedOllehMapStatus].busNumberNewDictionary objectForKeyGC:@"routeInfo"] count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // bis정류장리스트에서 유니크id가 없을 수도 있기 때문에
    // busrouteid를 확인해서 마을버스인지 일반버스인지 확인해본다
    NSString *routeId = stringValueOfDictionary(cellDic, @"busrouteid");
    ...
    if([uniqueId isEqualToString:@""] || [uniqueId isEqualToString:@"0"])
    {
        ...
        {
            float sequence = [numberValueOfDiction(oms.busNumberNewDictionary, @"direction_CHANGE_SEQ") floatValue];
            
            
            if(indexPath.row+1 < sequence)
            {
                [cell.busLineImg setImage:[UIImage imageNamed:@"bus_line_top.png"]];
            }
            else if (indexPath.row+1 == sequence)
            {
                [cell.busLineImg setImage:[UIImage imageNamed:@"bus_icon_s_turn.png"]];
                [cell.busLineImg setFrame:CGRectMake(289, 0, 25, 58)];
            }
            else if (sequence < 0)
            {
                [cell.busLineImg setImage:[UIImage imageNamed:@"bus_line_normal.png"]];
            }
            else
            {
                [cell.busLineImg setImage:[UIImage imageNamed:@"bus_line_bottom.png"]];
            }
...
- (void) saveRecentSearch
{
    ...
    NSString *busStr =  stringValueOfDictionary(oms.busNumberNewDictionary, @"bl_BUSNO");
    ...
    [busNumDic setObject:stringValueOfDictionary(oms.busNumberNewDictionary, @"laneid") forKey:@"ID"];
...
- (void)didFinishBusLineDrawCallBack:(id)request
{
    if ([request finishCode] == OMSRFinishCode_Completed)
	{
        ...
        if([(NSMutableDictionary *)[oms.busLineDrawingDictionary objectForKeyGC:@"BUSLINE"] count] == 0)
        {
            [OMMessageBox showAlertMessage:@"" :NSLocalizedString(@"Msg_BusNumber_NotDrawBusLine", @"")];
        }
        else
        {
            [MainViewController markingBusLineRoute_BusName:busNamed animated:NO];
        }
    }
    else if([[OllehMapStatus sharedOllehMapStatus] getNetworkStatus] == OMReachabilityStatus_disconnected)
    {
    ...
    NSMutableDictionary *fdic = [OMDatabaseConverter makeFavoriteDictionary:-1
                                                                  sortOrder:-1
                                                                   ...
                                                                   iconType:[oms getBusClassNumber:[[oms.busNumberNewDictionary objectForKeyGC:@"bl_BUSCLASS"] intValue]]
                                                                    ...
                                                                    coord3y:0
                                                                 detailType:@"TR_BUSNO"
                                                                   detailID:[oms.busNumberNewDictionary objectForKeyGC:@"laneid"] shapeType:@"" fcNm:@"" idBgm:@"" rdCd:@""];
