...
- (void) bottomViewFavorite:(NSDictionary *)dic placeHolder:(NSString *)placeHolderStr
{
    ...
    FavoAlertView *alert = [[FavoAlertView alloc] initWithTitle:@"즐겨찾기 이름" message:nil delegate:self cancelButtonTitle:@"취소" otherButtonTitles:@"확인", nil];
...
- (void)modalContact:(NSDictionary *)dic
{
    ...
    // 이름넣기
    ABRecordSetValue(person, kABPersonFirstNameProperty, (__bridge CFTypeRef)(contactName), nil);
    
    if(![contactImage isEqualToString:@""])
    {
        NSData *data1 = UIImagePNGRepresentation([oms urlGetImage:contactImage]);
        ABPersonSetImageData(person, (__bridge CFDataRef)data1, NULL);
    }
    ...
    {
        ABMultiValueAddValueAndLabel(homePage, (__bridge CFTypeRef)(contactHomePage), kABPersonHomePageLabel, NULL);
    }
    ...
    if(![contactTel isEqualToString:@""])
        ABMultiValueAddValueAndLabel(multiPhone, (__bridge CFTypeRef)(contactTel), kABPersonPhoneMainLabel, NULL);
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if([alertView isKindOfClass:[TelAlertView class]])
    {
        ...
        else if(buttonIndex == 1)
        {
            switch (_dicType)
            {
                ...
                case ParamType_TR:
                {
                    NSString *telNum = [[OllehMapStatus sharedOllehMapStatus].subwayDetailDictionary objectForKeyGC:@"SST_TEL"];
    ...
    else if([alertView isKindOfClass:[HomeAlertView class]])
    {
        switch (alertView.tag) {
            ...
            {
                if(buttonIndex == 1)
                    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://m.maxmovie.com"]];
            }
                break;
            default:
            {
    ...
    else if ([alertView isKindOfClass:[FavoAlertView class]])
    {
        if(buttonIndex == 1)
        {
            ...
            UITextField  *textfielder = [alertView textFieldAtIndex:0];
            ...
            switch (_dicType)
            {
                ...
                case ParamType_TR_BUS:
                {
                    ...
                    fdic = [OMDatabaseConverter makeFavoriteDictionary:-1 sortOrder:-1 category:Favorite_Category_Public title1:refinedText title2:ujName title3:@"" iconType:Favorite_IconType_BusStop coord1x:[[oms.busStationNewDictionary objectForKeyGC:@"bst_X"] doubleValue] coord1y:[[oms.busStationNewDictionary objectForKeyGC:@"bst_Y"] doubleValue] coord2x:0 coord2y:0 coord3x:0 coord3y:0 detailType:@"TR_BUS" detailID:[oms.busStationNewDictionary objectForKeyGC:@"stid"] shapeType:@"" fcNm:@"" idBgm:@"" rdCd:@""];
                }
                    break;
                case ParamType_TR_BUSNO:
                {
