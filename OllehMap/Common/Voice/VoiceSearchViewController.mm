...
- (void) initComponents_MainUI
{
    ...
    if (IS_4_INCH)
        [imgvwBack setImage:[UIImage imageNamed:@"voice_bg-568h.png"]];
    else
        [imgvwBack setImage:[UIImage imageNamed:@"voice_bg.png"]];
    
    [imgvwBack setFrame:CGRectMake(0, 37 + OM_STARTY, 320,
...
- (void) initComponents_VoiceRecognizer
{
    ...
    [[avrController sharedAVRController] Cache:YES];
    [[avrController sharedAVRController] Cache:NO];

    
    // 음성 인식 컨트롤 변수 설정
    //NSString *serverip = @"14.63.242.177";
    NSString *serverip = @"speechmap.ktgis.com";
    NSInteger serverport = 80;
    
    [[VoiceAssistController sharedVoiceAssist] Stop:Nil];
    
    //음성 인식 컨트롤러에 인자들을 넘긴다.
    NSInteger nRet;
    nRet = [[VoiceAssistController sharedVoiceAssist] SetParameter:serverip port:serverport Certification:@"1315413157" reqcontype:1];
    NSLog(@"@@@@@ SetParameter serverIP[%@] serverPort[%i]",
          serverip, serverport);
    ...
    {
        //[OMMessageBox showAlertMessage:@"" :@"시작못함"];
    }
    //
//    
//    // 음성 인식 컨트롤러에 인자들을 넘긴다.
...
-(void)notiConnectCannot:(NSNotification*) event
{
    ...
    if ( [vc isKindOfClass:[VoiceSearchViewController class]] )
        [OMMessageBox showAlertMessageTwoButtonsWithTitle:@"" message:NSLocalizedString(@"Msg_VoiceSearch_Controller_ServerConnectCannot", @"") target:self firstAction:@selector(closeVoiceSearchView) secondAction:@selector(retryVoiceSearchView) firstButtonLabel:@"닫기" secondButtonLabel:@"다시 시도"];
}
...
-(void)notiSessionTimeout:(NSNotification*) event
{
    ...
    if ( [vc isKindOfClass:[VoiceSearchViewController class]] )
    {
        [OMMessageBox showAlertMessageTwoButtonsWithTitle:@"" message:NSLocalizedString(@"Msg_VoiceSearch_Controller_SessionTimeout", @"") target:self firstAction:@selector(closeVoiceSearchView) secondAction:@selector(retryVoiceSearchView) firstButtonLabel:@"닫기" secondButtonLabel:@"다시 시도"];
        _searchFailed = YES;
    }
}
...
-(void)notiSuccessRecognize:(NSNotification*) event
{
    ...
    CSemParser *mpBnfParser = [[CSemParser alloc] init];
    ...
    if ([request finishCode] == OMSRFinishCode_Completed)
    {
        ...
        @try
        {
            ...
            if (countPlace >= 0 && countAddress >= 0 && countPublicBs >= 0 && countPublicBn >= 0 && countPublicSs >= 0 && countTotal > 0)
            {
                ...
                OMNavigationController *nc = [OMNavigationController sharedNavigationController];
                if ( ![[nc.viewControllers objectAtIndexGC:nc.viewControllers.count-1] isKindOfClass:[SearchResultViewController class]])
                {
