...
@synthesize prevSearchLocalDictionary = _prevSearchLocalDictionary;
...
@synthesize roadNameDictionary = _roadNameDictionary;
...
-(void) dealloc
{
    ...
    [_prevSearchLocalDictionary release];
    [_keyword release];
...
-(void) initStatus
{
    ...
    _prevSearchLocalDictionary = [[NSMutableDictionary alloc] init];
    ...
    // 도로명 초기화
    _roadNameDictionary = [[NSMutableDictionary alloc] init];
...
{
    return (int)[_noticeCheckArray count];
}
...
{
    return (int)[_recentSearch count];
}
...
- (void) trackEvent:(NSString *)eventAction
{
    id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker];
    
    [tracker send:[[GAIDictionaryBuilder createEventWithCategory:[NSString stringWithFormat:@"%@_category", eventAction]     // Event category (required)
                                                          action:[NSString stringWithFormat:@"%@_action", eventAction]  // Event action (required)
                                                           label:[NSString stringWithFormat:@"%@_label", eventAction]          // Event label
...
- (void) trackPageView:(NSString *)page
{
    ...
    [tracker set:kGAIScreenName value:[NSString stringWithFormat:@"%@", page]];
...
- (void) trackCustomVariable
{
    ...
    [tracker set:[GAIFields customDimensionForIndex:1] value:[[OllehMapStatus sharedOllehMapStatus] getDeviceModel]];
    [tracker set:[GAIFields customDimensionForIndex:2] value:[NSString stringWithFormat:@"%d", [[OllehMapStatus sharedOllehMapStatus] isRetinaDisplay]]];
    [tracker send:[[GAIDictionaryBuilder createAppView] build]];
...
-(BOOL) reSearchException
{
    ...
    if([option isEqualToString:use_option] || [[oms.searchLocalDictionary objectForKeyGC:@"SearchType"] count] < 1)
        return TRUE;
    else
        return FALSE;
}
