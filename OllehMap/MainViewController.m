...
- (void) viewWillAppear:(BOOL)animated
{
    ...
    NSLog(@"%@", NSStringFromCGRect(_kmapView.frame));
    
    [[MapContainer sharedMapContainer_Main] showMapContainer:_kmapView :self];
    
    if(!self.btnBottomTheme.selected)
        [self.eraseBtn setHidden:YES];
    else
    {
        [self.eraseBtn setBackgroundColor:[UIColor clearColor]];
        [self.eraseBtn setHidden:NO];
    }
    
    [self.view addSubview:_barBg];
...
- (void) viewDidAppear:(BOOL)animated
{
    ...
    if ( mc.kmap.trafficCCTV || mc.kmap.trafficBusStation || mc.kmap.trafficSubwayStation )
    {
        ...
        // arc넘어오면서 nil 줬다가 다시 셀프로 준다 delegate = nil;
    ...
    // 특화지도 실시간교통량 처음 실행시 안그려져서 최후의 수단

    // 이벤트 트래킹 - 메인 지도화면 노출
    if (_nMapRenderType == MapRenderType_Normal)
    {
        [[OllehMapStatus sharedOllehMapStatus] trackPageView:@"/main_map"];
        
        //[[OllehMapStatus sharedOllehMapStatus] trackEvent:@"/main_mapShowEvent"];
    }
}
...
- (void)viewDidLoad
{
    ...
    else if ( _nMapRenderType == MapRenderType_SearchResult_RoadCode)
        [self markingRoadInfo];
    
    [_kmapView setFrame:CGRectMake(0, 0, 320, self.view.frame.size.height)];
    NSLog(@"kmap frame : %@", NSStringFromCGRect(_kmapView.frame));
    ...
    if (special_up == Up_CCTV)
    {
        [self onOptionViewUseTrafficCCTV:selectedBtn];
    }
    if (special_up == Up_BusStation)
    {
        [self onOptionViewUseTrafficBusStation:selectedBtn];
    }
    if (special_up == Up_SubWay)
    {
        [self onOptionViewUseTrafficSubwayStation:selectedBtn];
    }
    
    UIButton *selectedBtn2 = [UIButton buttonWithType:UIButtonTypeCustom];
    [selectedBtn2 setSelected:NO];
    
    if (special_down == Down_Cadastral)
    {
        [self onOPtionViewUseTrafficAddress:selectedBtn2];
    }
    if (special_down == Down_Traffic)
    {
        [self onOptionViewUseTrafficInfo:selectedBtn2];
    }
...
- (void) appWillEnterForegroundNotification:(NSNotification *) event
{
...
- (int) getToastStartYValue
{
    return self.vwCurrentAddressGroup.frame.origin.y-10;
}
- (BOOL) first_toastShowing
{
    for (UIView *toastView in self.view.subviews)
    {
        if([toastView viewWithTag:1129])
        {
            return YES;
        }
...
- (BOOL) second_toastShowing
{
    for (UIView *toastView in self.view.subviews)
    {
        if([toastView viewWithTag:1130])
        {
            return YES;
        }
...
- (void) InitComponents
{
    ...
    if(OM_IOSVER)
    {
        _barBg = [[OMBlackStatusBar alloc] initWithFrame:CGRectMake(0, 0, 320, 20)];
    }
    ...
    _vwAutoUpdateContainer = [[UIView alloc]
                              initWithFrame:CGRectMake(0, OM_STARTY, 320,
    ...
    //[_vwNavigationbar insertSubview:imgvwBackground belowSubview:self.eraseBtn];
    ...
    // 검색결과 지도보기 화면일 경우
             ...
             || _nMapRenderType == MapRenderType_SearchResult_RoadCode)
    {
    ...
    _vwThemeUpdateContainer = [[UIView alloc]
                               initWithFrame:CGRectMake(0, 0 + OM_STARTY, 320,
                                                        self.view.frame.size.height - OM_STARTY)];
...
- (void) didFinishRequestRecommendWordVersion:(id)request
{
    ...
    if ([request finishCode] == OMSRFinishCode_Completed)
    {
        ...
        if (updateVersion > currentVersion)
        {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"업데이트 알림" message:updateAlertMessage delegate:self cancelButtonTitle:@"아니오" otherButtonTitles:@"예", nil];
            [alert setTag:0];
            [alert show];
...
- (void) callbackAutoUpdateExecute
{
...
- (void) onNoticePopup_Close:(id)sender
{
    ...
    {
        [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithInt:(int)btn.tag] forKey:@"NoticePopupLastSequence"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
...
- (void) toggleScreenMode :(int)mode :(BOOL)animated
{
    ...
    // 상단 네비게이션바(메인뷰전용) 위치 조정
    {
        ...
        switch (oms.currentMapScreenMode)
        {
            case MapScreenMode_FULL:
                rect.origin.y = -rect.size.height - _vwSearchGroup.frame.size.height - OM_STARTY; // -20.0f 추가된 이유는 최상단 스테이터스바 위로 올라가도록..
                break;
            case MapScreenMode_NORMAL:
                rect.origin.y = 0.0f + OM_STARTY;
                break;
        }
    ...
    // 상단 검색바 위치 조정
    {
        ...
        {
            case MapScreenMode_FULL:
                rect.origin.y = -rect.size.height - OM_STARTY; // -20.0f 추가된 이유는 최상단 스테이터스바 위로 올라가도록..
                break;
            case MapScreenMode_NORMAL:
                rect.origin.y =  42.0f + OM_STARTY; // _vwNavigationbar.frame.origin.y + _vwNavigationbar.frame.size.height + 5.0f; //네비게이션바 하단 5.0 좌표
                break;
        }
    ...
    // 내위치 버튼 위치 조정
    {
        ...
        {
            case MapScreenMode_FULL:
                rect.origin.y = 15.0f + OM_STARTY;
                break;
            case MapScreenMode_NORMAL:
                rect.origin.y = 91.0f + OM_STARTY;
                break;
        }
...
- (void) finishReversGeocodingAddress:(id)request
{
    ...
    if(mc.kmap.trafficInfo)
        [mc.kmap setTrafficInfo:mc.kmap.trafficInfo clearCache:YES];
    else if (mc.kmap.CadastralInfo)
        [mc.kmap setCadastralInfo:mc.kmap.CadastralInfo];
...
- (void) mapStatusChanged:(NSNumber *)mapLoad isZoom:(NSNumber *)isZoom
{
    ...
    if ( [mapLoad intValue] == 2 )
    {
        ...
        // 토스트 관련
        // 토스트는 customToast(먼저 뜨는 토스트)와 custom2Toast(나중에 뜨는 토스트) 가 있다
        // 테마관련 토스트는 customToast, 교통옵션일 때는 custom2Toast 사용
        // 하지만 특화지도가 bussation일 때(200m)는 busstation토스트가 customToast, 테마토스트는 custom2Toast 를 사용한다
        // 테마토스트는 보통 200~400m 이기 때문이다
        // 그래서 테마토스트를 띄울대, 특화지도가 busstation인 경우엔 다른 토스트를 띄우게 된다
        ...
        NSLog(@"%d  %d", mc.kmap.adjustZoomLevel, (int)themeMaxRenderingZoomLevel);
        ...
        else if (  mc.kmap.theme && mc.kmap.adjustZoomLevel < themeMaxRenderingZoomLevel )
        {
            ...
            {
                // 메세지 조합하기
                NSString *message = [NSString stringWithFormat:@"선택하신 테마는 %@ 이하에서만 제공됩니다.", [self convertToMeterStringFromZoomLevel:(int)themeMaxRenderingZoomLevel checkMapDisplay:NO]];

                int originYValue = [self getToastStartYValue];
                
                // 특화지도가 버스정류장인지 확인
                if(mc.kmap.trafficBusStation)
                {
                    if([self first_toastShowing])
                        originYValue = originYValue - 46;
                    
                    [self.view custom2Toast:message toastYValue:originYValue];
                }
                else
                    [self.view customToast:message toastYValue:originYValue];
        ...
        else if (_nMapRenderType == MapRenderType_SearchResult_LinePolyGon)
        {
            ...
            int vertextCnt = (int)[(NSMutableDictionary *)[[[[OllehMapStatus sharedOllehMapStatus].linePolygonDictionary objectForKeyGC:@"LinePolygon"] objectAtIndexGC:0] objectForKeyGC:@"part"] count];
            ...
            while (i < vertextCnt)
            {
                ...
                for (int i = 0;i<vertexArr.count - 1;i++)
                {
                    ...
                    Coord shortestCoord = [self betweenPointAndLine:myCrd pointA:compareDist1 pointB:compareDist2];
            ...
            NSLog(@"currentSingleOverlay : %@", _currentSingleOverlay ? @"YES" : @"NO");
            
            [self pinSearchSinglePOIOverlayWithLinePolygon:YES modiX:shortX modiY:shortY selected:_currentSingleOverlay == nil ? NO : YES];
        }
        else if (_nMapRenderType == MapRenderType_SearchResult_RoadCode)
        {
            _currentLongTapOverlay = nil;
            
            int vertextCnt = (int)[(NSMutableDictionary *)[[[[OllehMapStatus sharedOllehMapStatus].roadNameDictionary objectForKeyGC:@"RoadCode"] objectAtIndexGC:0] objectForKeyGC:@"part"] count];
            ...
            while (i < vertextCnt)
            {
                ...
                NSArray *vertexArr = [[[[[[OllehMapStatus sharedOllehMapStatus].roadNameDictionary objectForKeyGC:@"RoadCode"] objectAtIndexGC:0] objectForKeyGC:@"part"] objectAtIndexGC:i] objectForKeyGC:@"vertex"];
            ...
            [self pinSearchSinglePOIOverlayWithRoadName:YES modiX:shortX modiY:shortY selected:_currentSingleOverlay == nil ? NO : YES];
        }
        ...
        else if ( mc.kmap.trafficBusStation && mc.kmap.adjustZoomLevel < 9 )
        {
            ...
            // 이전 줌레벨 수치 보정 (작은글씨지도일때..)
            NSInteger adjustLastZoomLevel = mc.kmap.lastMapZoomLevel;
            
            if ( [isZoom boolValue] && adjustLastZoomLevel >= 9 )
            {
                NSString *message = [NSString stringWithFormat:@"버스정류장은 200m 이하에서만 제공됩니다."];

                [self.view customToast:message toastYValue:[self getToastStartYValue]];
            }
        ...
        else if ( mc.kmap.trafficSubwayStation && mc.kmap.adjustZoomLevel < 7 )
        {
            ...
            if ( [isZoom boolValue] && adjustLastZoomLevel >= 7 )
            {
                // 메세지 조합하기
                NSString *message = [NSString stringWithFormat:@"지하철은 800m 이하에서만 제공됩니다."];
        ...
        else if ( mc.kmap.trafficCCTV && mc.kmap.adjustZoomLevel < 4 )
        {
            ...
            if ( [isZoom boolValue] && adjustLastZoomLevel >= 4 )
            {
            // 메세지 조합하기
                NSString *message = [NSString stringWithFormat:@"CCTV는 6km 이하에서만 제공됩니다."];
        ...
        if (mc.kmap.CadastralInfo && mc.kmap.adjustZoomLevel < cadastralMaxLevel)
        {
            ...
            NSLog(@"이전 줌레벨 : %d", (int)adjustLastZoomLevel);
            ...
            {
                // 메세지 조합하기
                NSString *message = [NSString stringWithFormat:@"지적편집도는 %@ 이하에서만 제공됩니다.", [self convertToMeterStringFromZoomLevel:mc.kmap.adjustZoomLevel+1 checkMapDisplay:NO]];
...
- (void) callbackThemesRequest :(NSTimer*)timer
{
    ...
    if ( requestType == 0  && requestTime == _trafficOptionLastRequestTime )
    {
        ...
        else if ( trafficType == 1 ) // 버스정류장
        {
            ...
            if ( isZoom ) [mc.kmap removeAllTrafficOverlay];
            // 검색요청
            [[ServerConnector sharedServerConnection] requestTrafficOptionBusStationList:self action:@selector(finishTrafficOptionBusStationList:) coordidate:mc.kmap.centerCoordinate radius:(int)mc.getCurrentMapZoomLevelMeterWithScreen/2];
        }
        else if ( trafficType == 2 )
        {
            ...
            if ( isZoom ) [mc.kmap removeAllTrafficOverlay];
            // 검색요청
            [[ServerConnector sharedServerConnection] requestTrafficOptionSubwayStationList:self action:@selector(finishTrafficOptionSubwayStationList:) coordidate:mc.kmap.centerCoordinate radius:(int)mc.getCurrentMapZoomLevelMeterWithScreen/2];
        }
    }
    // 테마 요청인 경우...
    else if ( requestType == 1 && requestTime == _themeLastRequestTime )
    {
        ...
        // 검색요청
        [[ServerConnector sharedServerConnection] requestThemeDetail:self action:@selector(didFinishRequestThemeSearch:) themeCode:themeCode pX:mc.kmap.centerCoordinate.x pY:mc.kmap.centerCoordinate.y radius:(int)[mc getCurrentMapZoomLevelMeterWithScreen]/2];
    }
...
- (void) pinRouteStartPOIOverlay
{
    ...
    [overlay setCenterOffset:CGPointMake(overlay.imageSize.width/2, overlay.imageSize.height)];
...
- (void) pinLongtapPOIOverlay:(BOOL)isDisplay :(BOOL)animatedMarkerOption
{
    ...
    if ( isDisplay )
    {
        ...
        if(_currentSingleOverlay != nil)
        {
            [mc.kmap selectPOIOverlay:nil];
            _currentSingleOverlay = nil;
        }
        else if (_currentMultiOverlay != nil)
        {
            [mc.kmap selectPOIOverlay:nil];
            _currentMultiOverlay = nil;
        }
        ...
        //[OMMessageBox showAlertMessage:@"" :[NSString stringWithFormat:@"%f %f", oms.searchResultOneTouchPOI.coordLocationPoint.x, oms.searchResultOneTouchPOI.coordLocationPoint.y]];
...
- (void) pinPOIMarkerOption:(BOOL)isDisplay targetInfo:(NSDictionary *)targetInfo duplicatedInfo:(NSDictionary*)duplicatedInfo animated:(BOOL)animated
{
    ...
    if ( isDisplay )
    {
        ...
        //[searchRouteStartButton setEnabled:[stringValueOfDictionary(targetInfo, @"Shape") isEqualToString:@""] ? YES : NO];
        if(![stringValueOfDictionary(targetInfo, @"Shape") isEqualToString:@""] || ![stringValueOfDictionary(targetInfo, @"RdCd") isEqualToString:@""])
        {
            [searchRouteStartButton setImage:[UIImage imageNamed:@"map_btn_start_disabled.png"] forState:UIControlStateNormal];
            [searchRouteStartButton setTag:1000];
        }
        ...
        if(![stringValueOfDictionary(targetInfo, @"Shape") isEqualToString:@""] || ![stringValueOfDictionary(targetInfo, @"RdCd") isEqualToString:@""])
        {
            [searchRouteDestButton setImage:[UIImage imageNamed:@"map_btn_stop_disabled.png"] forState:UIControlStateNormal];
            [searchRouteDestButton setTag:1001];
        }
        ...
        if(![stringValueOfDictionary(targetInfo, @"Shape") isEqualToString:@""] || ![stringValueOfDictionary(targetInfo, @"RdCd") isEqualToString:@""])
        {
            [searchRouteVisitButton setTag:1002];
            [searchRouteVisitButton setImage:[UIImage imageNamed:@"map_btn_via_disabled.png"] forState:UIControlStateNormal];
        }
...
- (void) pinSearchSinglePOIOverlay:(BOOL)isDisplay
{
    ...
    if (isDisplay)
    {
        ...
        _currentSingleOverlay = (OMImageOverlaySearchSingle *)overlay;
        _currentMultiOverlay = nil;
        _currentLongTapOverlay = nil;
        ...
        [overlay.additionalInfo setObject:oms.searchResult.strLocationRoadName forKey:@"RdCd"];
...
- (void) pinSearchResultLinePOIOverlay:(BOOL)isDisplay
{
    // 검색결과 단일 POI 처리
    ...
    if (isDisplay)
    {
        ...
        // 오버레이 생성
        OMImageOverlayLineSingle *overlay = nil;
        
        overlay = [[OMImageOverlayLineSingle alloc] initWithImage:[UIImage imageNamed:@"line_map_b_marker_pressed.png"]];
        overlay.usePOIIcon = NO;
...
- (void) pinSearchLinePOIOverlay:(BOOL)isDisplay
{
    // 검색결과 단일 POI 처리
    ...
    if (isDisplay)
    {
        ...
        // 현재 인덱스가 0보다 클경우에는 알파벳 처리, 아닌경우..
        OMImageOverlayLineSingle *overlay = [[OMImageOverlayLineSingle alloc] initWithImage:[UIImage imageNamed:@"line_map_b_marker_poi_pressed.png"]];
//        // 오버레이 생성
//        OMImageOverlayLineSingle *overlay = nil;
//        
//        if ( oms.searchResult.index > 0 )
//        {
//            overlay = [[OMImageOverlayLineSingle alloc] initWithImage:[UIImage imageNamed:@"line_map_b_marker_pressed.png"]];
//            overlay.lineUsePOIIcon = NO;
//        }
//        else
//        {
//            overlay = [[OMImageOverlayLineSingle alloc] initWithImage:[UIImage imageNamed:@"line_map_b_marker_poi_pressed.png"]];
//            overlay.lineUsePOIIcon = YES;
//        }
//        // 알파벳 이미지 추가
//        UIImageView *searchIndexImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[NSString stringWithFormat:@"marker_%d.png", oms.searchResult.index]]];
//        
...
//        
        ...
        [overlay.additionalInfo setObject:[NSNumber numberWithFloat:oms.searchResult.coordLocationPoint.y] forKey:@"Y"];
        [overlay.additionalInfo setObject:oms.searchResult.strLocationRoadName forKey:@"RdCd"];
        [overlay.additionalInfo setObject:oms.searchResult.strShape forKey:@"Shape"];
...
- (void) pinSearchMultiPOIOverlay:(BOOL)isDisplay
{
    ...
    if (isDisplay)
    {
        ...
        for (NSDictionary *duplicatedPOI in duplicatedPOIList)
        {
            ...
            int dupCount = (int)[(NSMutableDictionary *)[duplicatedPOI objectForKeyGC:@"POIs"] count];
...
- (void) pinRecentPOIOverlay:(BOOL)isDisplay
{
    ...
    if (isDisplay)
    {
        ...
        _currentSingleOverlay = (OMImageOverlayRecent *)overlay;
        _currentMultiOverlay = nil;
        _currentLongTapOverlay = nil;
...
{
    LegendViewController *lvc = [self.storyboard instantiateViewControllerWithIdentifier:@"LegendView"];
    [[OMNavigationController sharedNavigationController] pushViewController:lvc animated:YES];
}
...
- (void) pinFavoritePOIOverlay:(BOOL)isDisplay
{
    ...
    if (isDisplay)
    {
        ...
        _currentSingleOverlay = (OMImageOverlayFavorite *)overlay;
        _currentMultiOverlay = nil;
        _currentLongTapOverlay = nil;
...
- (void) pinSearchSinglePOIOverlayWithLinePolygon:(BOOL)isDisplay modiX:(float)x modiY:(float)y selected:(BOOL)selected
{
    // 검색결과 단일 POI 처리
    ...
    if (isDisplay)
    {
        ...
        if(selected)
        {
            ...
            _currentSingleOverlay = (OMImageOverlayLineSingle *)overlay;
            _currentMultiOverlay = nil;
            _currentLongTapOverlay = nil;
        }
        else
...
- (void) pinSearchSinglePOIOverlayWithRoadName:(BOOL)isDisplay modiX:(float)x modiY:(float)y selected:(BOOL)selected
{
    // 검색결과 단일 POI 처리
...
- (void) markingSinglePOI
{
    ...
    // 즐겨찾기나 최근검색일 경우
    if (_nMapRenderSinglePOICategory == MainMap_SinglePOI_Type_Favorite
        || _nMapRenderSinglePOICategory == MainMap_SinglePOI_Type_Recent )
    {
        if ( _nMapRenderSinglePOICategory == MainMap_SinglePOI_Type_Recent )
            [self pinRecentPOIOverlay:YES];
        else if (_nMapRenderSinglePOICategory == MainMap_SinglePOI_Type_Favorite )
    ...
    else if (_nMapRenderSinglePOICategory == MainMap_SinglePOI_Type_Line)
    {
        [self pinSearchResultLinePOIOverlay:YES];
    }
    // 일반적인 SinglePOI 인 경우
    else if ( _nMapRenderSinglePOICategory == MainMap_SinglePOI_Type_Normal)
    {
...
- (void) markingLinePolyGon
{
    ...
    int vertexCnt = (int)[(NSMutableDictionary *)[[[oms.linePolygonDictionary objectForKeyGC:@"LinePolygon"] objectAtIndexGC:0] objectForKeyGC:@"part"] count];
    
    if ( [oms.linePolygonDictionary count] <= 0 )
    {
        [OMMessageBox showAlertMessage:@"" :@"라인POI 정보가 존재하지 않습니다."];
        [[OMNavigationController sharedNavigationController] popViewControllerAnimated:NO];
        return;
    }
    ...
    [self pinSearchLinePOIOverlay:YES];
    //[self pinSearchSinglePOIOverlay:YES];
    ...
    while (i < vertexCnt)
    {
        ...
        CGColorRef stroke = CGColorCreate(rgb, (CGFloat[]){convertHexToDecimal(@"67") ,convertHexToDecimal(@"17") ,convertHexToDecimal(@"d2") ,1.0f});
...
- (void) markingRoadInfo
{
    ...
    int vertexCnt = (int)[(NSMutableDictionary *)[[[oms.roadNameDictionary objectForKeyGC:@"RoadCode"] objectAtIndexGC:0] objectForKeyGC:@"part"] count];
    
    if ( [oms.roadNameDictionary count] <= 0 )
    {
        [OMMessageBox showAlertMessage:@"" :@"도로명 정보가 존재하지 않습니다."];
        [[OMNavigationController sharedNavigationController] popViewControllerAnimated:NO];
        return;
    }
    ...
    while (i < vertexCnt)
    {
        NSArray *vertexArr = [[[[[oms.roadNameDictionary objectForKeyGC:@"RoadCode"] objectAtIndexGC:0] objectForKeyGC:@"part"] objectAtIndexGC:i] objectForKeyGC:@"vertex"];
...
- (void) markingBusLineRoute
{
    ...
    // 경로 색상
    CGColorSpaceRef rgb = CGColorSpaceCreateDeviceRGB();
    CGColorRef stroke = CGColorCreate(rgb, (CGFloat[]){convertHexToDecimal(@"00") ,convertHexToDecimal(@"18") ,convertHexToDecimal(@"ff") , 0.8f});
    plOverlay.strokeColor = stroke;
    CGColorSpaceRelease(rgb);
    CFRelease(stroke);
    // 경로 나머지 설정
...
#else
            //[toast setObject:@"CCTV POI 를 그리는데 실패했습니다." forKey:@"Message"];
#endif
...
- (void) onPOIMarkerOptionDetailButton:(id)sender
{
    ...
    NSString *shareRdCd = stringValueOfDictionary(additionalInfo, @"RdCd");
    ...
    // 이벤트 트래킹 - 검색 후 상세보기
    else if (_nMapRenderType == MapRenderType_SearchResult_SinglePOI || _nMapRenderType == MapRenderType_SearchResult_MultiPOI || _nMapRenderType == MapRenderType_SearchResult_LinePolyGon || _nMapRenderType == MapRenderType_SearchResult_RoadCode)
    ...
    if (nc.viewControllers.count > 1 && (_nMapRenderType == MapRenderType_SearchResult_SinglePOI || _nMapRenderType == MapRenderType_SearchResult_LinePolyGon || _nMapRenderType == MapRenderType_SearchResult_RoadCode) && _selectedMultiPOIIndex == -1)
    {
        ...
        if ([vc isKindOfClass:[CommonPOIDetailViewController class]])
        {
            ...
            // 지도보기 후 또 지도보기
            [[MapContainer sharedMapContainer_Main].kmap setDelegate:nil];
            // 화면 나가기
    ...
    {
        AddressPOIViewController *avc = [self.storyboard instantiateViewControllerWithIdentifier:@"AddressPOIView"];
        avc.poiZibunAddress = [NSString stringWithFormat:@"%@", shareName];
        avc.poiRoadAddress = [NSString stringWithFormat:@"%@", shareNewAddress];
        avc.poiCrd = shareCoord;
        avc.oldOrNew = [NSString stringWithFormat:@"%@",shareOldorNew];
        avc.rdCd = [NSString stringWithFormat:@"%@", shareRdCd];
    ...
    switch ((enum OMStatus_TouchesType)oms.currentTouchesType)
    {
        case TouchesType_TAP:
            ...
            else if (_currentSingleOverlay)
            {
                ...
                [self pinPOIMarkerOption:NO targetInfo:_currentSingleOverlay.additionalInfo animated:YES];
                [mc.kmap selectPOIOverlay:nil];
                [self currentOverlayNoSelected];
            ...
            else if (_currentMultiOverlay)
            {
                ...
                [self pinPOIMarkerOption:NO targetInfo:_currentMultiOverlay.additionalInfo animated:YES];
                [mc.kmap selectPOIOverlay:nil];
                [self currentOverlayNoSelected];
...
- (void) overlayTouched:(Overlay *)overlay
{
    ...
    if ( [overlay isKindOfClass:[OMImageOverlay class]] )
    {
        ...
        // 검색결과 라인POI
        else if ( [overlay isMemberOfClass:[OMImageOverlayLineSingle class]] )
        {
            // 검색결과 단일POI가 클린된 경우
            OMImageOverlayLineSingle *searchSingleOverlay = (OMImageOverlayLineSingle*)overlay;
            ...
            else
            {
                ...
                _currentSingleOverlay = (OMImageOverlaySearchSingle *)searchSingleOverlay;
                _currentLongTapOverlay = nil;
            }
        }
        ...
        else if ( [overlay isMemberOfClass:[OMImageOverlaySearchMulti class]] )
        {
            ...
            else
            {
                ...
                if ( searchMultiOverlay.duplicated )
                {
                    ...
                    [self showDuplicatedPOIList:searchMultiOverlay title:@"선택 목록"];
                }
                // 단일 POI 선택한 경우
                else
                {
                    ...
                    _currentMultiOverlay = searchMultiOverlay;
                    _currentLongTapOverlay = nil;
                    _currentSingleOverlay = nil;
                }
        ...
        else if ( [overlay isMemberOfClass:[OMImageOverlayTrafficCCTV class]] )
        {
            ...
            else
            {
                ...
                {
                    [self showDuplicatedPOIList:cctvOverlay title:@"CCTV"];
                }
                // 단일 POI
                else
                {
        ...
        else if ( [overlay isMemberOfClass:[OMImageOverlayTrafficBusStation class]] )
        {
            ...
            else
            {
                ...
                {
                    [self showDuplicatedPOIList:busStationOverlay title:@"버스정류장"];
                }
                // 단일 POI 선택한 경우
                else
                {
        ...
        else if ( [overlay isMemberOfClass:[OMImageOverlayTrafficSubwayStation class]] )
        {
            ...
            else
            {
                ...
                {
                    [self showDuplicatedPOIList:subwayOverlay title:@"지하철"];
                }
                // 단일 POI 선택한 경우
                else
                {
        ...
        else if ( [overlay isMemberOfClass:[OMImageOverlayTheme class]] )
        {
            ...
            else
            {
                ...
                {
                    [self showDuplicatedPOIList:themeOverlay title:@""];
                }
                // 단일 POI 선택한 경우
                else
                {
...
- (void) didFinishRequestAllPOIDetail :(id)request
{
    if ([request finishCode] == OMSRFinishCode_Completed)
    {
        ...
        // 이건필요없어보임...oms.searchResult의 RDCD도 필요없을 듯?
        else if (_nMapRenderType == MapRenderType_SearchResult_RoadCode)
        {
        ...
        else if ([detailType isEqualToString:@"MP"])
        {
            ...
            if(![requestType isEqualToString:detailType] && [requestType isEqualToString:@"MV"])
            {
                ...
                NSString *name = stringValueOfDictionary(additionalInfo, @"Name");
                //
                [mpdvc setThemeToDetailName:name];
                [mpdvc setDisplayMapBtn:_nMapRenderType == MapRenderType_Normal];
                [mpdvc setDisplayMapBtn:NO];
            ...
            else if(![requestType isEqualToString:detailType] && [requestType isEqualToString:@"OL"])
            {
                ...
                [opdvc setDisplayMapBtn:_nMapRenderType == MapRenderType_Normal];
                [opdvc setDisplayMapBtn:NO];
            ...
            else
            {
                ...
                NSString *name = stringValueOfDictionary(additionalInfo, @"Name");
                [gpdvc setDisplayMapBtn:_nMapRenderType == MapRenderType_Normal];
                [gpdvc setDisplayMapBtn:NO];
        ...
        else if ([detailType isEqualToString:@"TR"])
        {
            ...
            SubwayPOIDetailViewController *vc = [self.storyboard instantiateViewControllerWithIdentifier:@"SubwayPOIDetailView"];
        ...
        {
            BusStationPOIDetailViewController *vc = [self.storyboard instantiateViewControllerWithIdentifier:@"BusStationPOIDetailView"];
...
-(Coord) betweenPointAndLine :(Coord)anotherC pointA:(Coord)lineOnA pointB:(Coord)lineOnB
{
    double xx = 0.0;
    double yy = 0.0;
...
- (void) onSelectDuplicatePOI:(id)sender
{
    ...
    // 마커옵션 활성화
    _selectedMultiPOIIndex = (int)index;
    ...
    _currentMultiOverlay = (OMImageOverlaySearchMulti *)overlay;
    _currentLongTapOverlay = nil;
    _currentSingleOverlay = nil;
...
- (void) onOPtionViewUseTrafficAddress:(id)sender
{
    ...
    if(mc.kmap.mapDisplay == KMapDisplayHD)
    {
        ...
        [closePopupView setFrame:CGRectMake(76/2, 212/2 - 40/2, 488/2, 364/2)];
        
        
        UIImageView *closePopupBg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"popup_bg.png"]];
        [closePopupBg setFrame:CGRectMake(0, 0, 488/2, 364/2)];
        [closePopupView addSubview: closePopupBg];
        
        // 타이틀 렌더링
        {
            UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 488/2, 62/2)];
            ...
            UIButton *titleBtn = [[UIButton alloc] initWithFrame:CGRectMake(4/2 + 430/2, 10/2, 40/2, 40/2)];
            [titleBtn setImage:[UIImage imageNamed:@"btn_close.png"] forState:UIControlStateNormal];
            [titleView addSubview:titleBtn];
            [titleBtn addTarget:self action:@selector(dimmedPopUpClose:) forControlEvents:UIControlEventTouchUpInside];
            
            UIImageView *titleUnderLine = [[UIImageView alloc] initWithFrame:CGRectMake(4/2, 60/2, 480/2, 2/2)];
            [titleUnderLine setImage:[UIImage imageNamed:@"popup_title_line.png"]];
            [titleView addSubview:titleUnderLine];
     
            
            [closePopupView addSubview:titleView];
        
        }
        
        
        UILabel *closeLabel = [[UILabel alloc] init];
        [closeLabel setFrame:CGRectMake(0, 62/2, 246, 96)];
        ...
        [closePopUpBtn setImage:[UIImage imageNamed:@"popup_btn_close.png"] forState:UIControlStateNormal];
        [closePopUpBtn setFrame:CGRectMake((250-76) / 2, (42+172+48)/2, 70, 31)];
    ...
    [_btnBottomLegend setHidden:!trafficAddressButton.selected];
    
    if(trafficAddressButton.selected)
    {
        [[NSUserDefaults standardUserDefaults] setInteger:Down_Cadastral forKey:@"private_down"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
    else
    {
    ...
    NSLog(@"지적도 : %@", trafficAddressButton.selected ? @"on" : @"off");
...
- (void) onOptionViewUseTrafficInfo:(id)sender
{
    ...
    if(trafficInfoButton.selected)
    {
        [[NSUserDefaults standardUserDefaults] setInteger:Down_Traffic forKey:@"private_down"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
    else
    {
...
- (void) onOptionViewUseTrafficCCTV:(id)sender
{
    ...
    if(trafficCCTVButton.selected)
    {
        [[NSUserDefaults standardUserDefaults] setInteger:Up_CCTV forKey:@"private_up"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
    else
    {
...
- (void) onOptionViewUseTrafficBusStation:(id)sender
{
    ...
    if(trafficBusStationButton.selected)
    {
        [[NSUserDefaults standardUserDefaults] setInteger:Up_BusStation forKey:@"private_up"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
    else
    {
...
- (void) onOptionViewUseTrafficSubwayStation:(id)sender
{
    ...
    if(trafficSubwayStationButton.selected)
    {
        [[NSUserDefaults standardUserDefaults] setInteger:Up_SubWay forKey:@"private_up"];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
    else
    {
...
- (void) finishTrafficOptionCCTVInfo :(ServerRequester*)request
{
    if ( [request finishCode] == OMSRFinishCode_Completed )
    {
        ...
        CCTVViewController *cctvVC = [self.storyboard instantiateViewControllerWithIdentifier:@"CCTVView"];
...
- (void) renderRealtimeTrafficBusTimeTable :(NSDictionary*)timetable :(NSString*)busid
{
    ...
    // 컨테이너 삽입
    [_kmapView addSubview:_vwRealtimeTrafficTimeTableContainer];
}
...
// 교통옵션 다중POI 팝업호출
- (void) showDuplicatedPOIList:(OMImageOverlay *)overlay title:(NSString *)title
{
    ...
    // 팝업 리스트 뷰 생성
    UIView *vwMultiPOISelector = [[UIView alloc] initWithFrame:CGRectMake(116/2, 250/2, 410/2, 458/2)];
    ...
    // 스크롤뷰 생성
    OMScrollView *svwList = [[OMScrollView alloc] initWithFrame:CGRectMake(4/2, 62/2, 402/2, 141)];
    ...
    // 타이틀 렌더링
    {
        ...
        if ( [overlay isMemberOfClass:[OMImageOverlayTheme class]])
            [titleLbl setText:stringValueOfDictionary([ThemeCommon sharedThemeCommon].additionalInfo, @"SelectedThemeName")];
        else
        ...
        [titleBtn setImage:[UIImage imageNamed:@"btn_close.png"] forState:UIControlStateNormal];
        [titleView addSubview:titleBtn];
        [titleBtn addTarget:self action:@selector(onCloseDuplicatePOIList:) forControlEvents:UIControlEventTouchUpInside];
    ...
    for (NSMutableDictionary *poiDic in list)
    {
        ...
        // 라벨
        CGRect rectName = CGRectMake(80/2, 30/2, 150, 30/2);
        ...
        else if ( [overlay isMemberOfClass:[OMImageOverlayTheme class]])
        {
            ...
            //            // 테마마커 임시변경
            //            NSString *imagePathSingle = [[NSBundle mainBundle] pathForResource:@"list_b_marker.png" ofType:nil];
            //            UIImage *themeImageSingle =[UIImage imageWithContentsOfFile:imagePathSingle];
        ...
        // 라인 삽입
        UIImageView *vwLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, listContentsHeight, svwList.frame.size.width, 2)];
    ...
    // 닫기 버튼
    UIButton *btnClose = [[UIButton alloc] initWithFrame:CGRectMake(116/2, 364/2, 176/2, 64/2)];
...
- (void) didFinishRequestThemeInfoList:(id)request
{
    if([request finishCode] == OMSRFinishCode_Completed)
    {
        ...
        {
            ThemeViewController *tvc = [self.storyboard instantiateViewControllerWithIdentifier:@"ThemeView"];
            
            [[OMNavigationController sharedNavigationController] pushViewController:tvc animated:YES];
...
- (void) finishNoticeListUICallBack:(id)request
{
    ...
    SettingViewController *svc = [self.storyboard instantiateViewControllerWithIdentifier:@"SettingView"];
...
{
    SearchViewController *svc = [self.storyboard instantiateViewControllerWithIdentifier:@"SearchView"];
    [[OMNavigationController sharedNavigationController] pushViewController:svc animated:YES];
}
...
+ (void) markingThemePOI_ThemeCode:(NSString*)themeCode mainThemeCode:(NSString*)mainThemeCode maxRenderingZoomLevel:(int)maxRenderingZoomLevel selectedThemeName:(NSString *)selectedThemeName animated:(BOOL)animated
{
    ...
    [[ThemeCommon sharedThemeCommon].additionalInfo  setObject:selectedThemeName forKey:@"SelectedThemeName"];
    
    
    
    // 메인지도를 불러온다.
    MainViewController *mmvc = (MainViewController*)[[OMNavigationController sharedNavigationController].viewControllers objectAtIndexGC:0];
    ...
    [mmvc.eraseBtn setBackgroundColor:[UIColor clearColor]];
    [mmvc.eraseBtn setHidden:NO];
    ...
    [[ServerConnector sharedServerConnection] requestThemeDetail:mmvc action:@selector(didFinishRequestThemeSearch:) themeCode:themeCode pX:mc.kmap.centerCoordinate.x pY:mc.kmap.centerCoordinate.y radius:(int)mc.getCurrentMapZoomLevelMeterWithScreen/2];
}
...
//                [toast setObject:[NSNumber numberWithBool:YES] forKey:@"AutoClose"];
                ...
                if(mc.kmap.trafficBusStation)
                {
                    ...
                    [self.view custom2Toast:@"선택하신 테마가 현재 지도화면에 없습니다." toastYValue:originYValue];
                }
                else
                    [self.view customToast:@"선택하신 테마가 현재 지도화면에 없습니다." toastYValue:originYValue];
            ...
            // 테마마커 임시변경
//            NSString *imagePathSingle = [[NSBundle mainBundle] pathForResource:@"map_b_marker_poi.png" ofType:nil];
//            UIImage *themeImageSingle =[UIImage imageWithContentsOfFile:imagePathSingle];
//            
//            NSString *imagePathDup = [[NSBundle mainBundle] pathForResource:@"map_b_marker_overlap.png" ofType:nil];
//            UIImage *themeImageDuplicated =[UIImage imageWithContentsOfFile:imagePathDup];
...
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(alertView.tag == 0)
    {
        switch (buttonIndex) {
            case 0:
            {
                [self callbackAutoUpdateCancel];
                break;
            }
            case 1:
            {
                [self callbackAutoUpdateExecute];
                break;
            }
            default:
                break;
...
+ (void) markingSinglePOI_RenderType:(int)type category:(int)category animated:(BOOL)animated
{
    ...
    if ( category == MainMap_SinglePOI_Type_Recent || category == MainMap_SinglePOI_Type_Favorite
    ...
    // 메인맵 컨테이너 생성
    MainViewController *main = [sboard instantiateViewControllerWithIdentifier:@"MainView"];
    main.nMapRenderType = type;
    main.nMapRenderSinglePOICategory = category;
...
+ (void) markingMultiPOI_RenderType:(int)type animated:(BOOL)animated
{
    ...
    // 메인맵 컨테이너 생성
    MainViewController *main = [sboard instantiateViewControllerWithIdentifier:@"MainView"];
    main.nMapRenderType = MapRenderType_SearchResult_MultiPOI;
    [main setMultiPOIMarkingType:type];
...
+ (void) markingBusLineRoute_BusName:(NSString*)busname animated:(BOOL)animated
{
    ...
    main.nMapRenderType = MapRenderType_SearchResult_BusLineRoute;
...
+ (void) markingLinePolygonPOI
{
    ...
    // 메인맵 컨테이너 생성
    MainViewController *main = [sboard instantiateViewControllerWithIdentifier:@"MainView"];
    main.nMapRenderType = MapRenderType_SearchResult_LinePolyGon;
...
+ (void) markingRoadCodeSearch
{
    ...
    // 메인맵 컨테이너 생성
    MainViewController *main = [sboard instantiateViewControllerWithIdentifier:@"MainView"];
    main.nMapRenderType = MapRenderType_SearchResult_RoadCode;
