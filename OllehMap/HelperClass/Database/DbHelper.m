...
+ (NSMutableDictionary*) makeFavoriteDictionary :(int)favoriteID sortOrder:(int)sortOrder category:(int)category title1:(NSString*)title1 title2:(NSString*)title2 title3:(NSString*)title3 iconType:(int)iconType coord1x:(double)coord1x coord1y:(double)coord1y coord2x:(double)coord2x coord2y:(double)coord2y coord3x:(double)coord3x coord3y:(double)coord3y detailType:(NSString *)detailType detailID:(NSString *)detailID shapeType:(NSString *)shapeType fcNm:(NSString *)fcNm idBgm:(NSString *)idBgm rdCd:(NSString *)rdCd;
{
    ...
    @try
    {
        ...
        [favoriteDic setObject:rdCd forKey:@"RdCd"];
...
- (void) columnAddCheck3
{
    sqlite3_stmt *sqlStatement;
    NSString *sql = [NSString stringWithFormat:@"SELECT * FROM sqlite_master where sql like('%%%@%%')", @"RdCd"];
    if(sqlite3_prepare(_favoriteListDatabase, [sql UTF8String] , -1, &sqlStatement, NULL) != SQLITE_OK)
    {
        NSLog(@"Problem with prepare statement tableInfo %@",[NSString stringWithUTF8String:(const char *)sqlite3_errmsg(_favoriteListDatabase)]);
    ...
    if(returnSQL == nil)
    {
        NSString *rdCdQuery = [NSString stringWithFormat:@"ALTER TABLE FavoriteListTB add column RdCd text"];
        sqlite3_exec(_favoriteListDatabase, [rdCdQuery UTF8String], nil, nil, nil);
...
- (void) getFavoriteList
{
    ...
    // 컬럼추가(RdCd)
    [self columnAddCheck3];
    
    NSString *query = [NSString stringWithFormat:@"SELECT FavoriteID,SortOrder,Category,Title1,Title2,Title3,IconType,Coord1x,Coord1y,Coord2x,Coord2y,Coord3x,Coord3y,DetailType,DetailID,ShapeType,FcNm,IdBgm,RdCd FROM FavoriteListTB WHERE 1=1 ORDER BY CAST(SortOrder AS NUMERIC) ASC, CAST(Category AS NUMERIC) ASC"];
    //NSLog(@"%@", query);
    ...
    if (sqlite3_prepare_v2(_favoriteListDatabase, [query UTF8String], -1, &selectStatement, NULL) == SQLITE_OK)
    {
        ...
        while (sqlite3_step(selectStatement) == SQLITE_ROW)
        {
            ...
            char *rdCdrow = (char *)sqlite3_column_text(selectStatement, 18);
            NSString *rdCd = rdCdrow ? [NSString stringWithUTF8String:rdCdrow] : [NSString string];
            
            // 즐겨찾기 정보 생성
            NSMutableDictionary *fDic = [OMDatabaseConverter makeFavoriteDictionary:favoriteID sortOrder:sortOrder category:category title1:title1 title2:title2 title3:title3 iconType:iconType coord1x:coord1x coord1y:coord1y coord2x:coord2x coord2y:coord2y coord3x:coord3x coord3y:coord3y detailType:detailType detailID:detailID shapeType:shapeType fcNm:fcNm idBgm:idBgm rdCd:rdCd];
...
- (BOOL) favoriteValidCheck:(NSDictionary *)favoriteDic
{
    ...
    NSLog(@"즐겨찾기 카운트 : %d", (int)[(NSMutableArray *)[OllehMapStatus sharedOllehMapStatus].favoriteList count]);
...
- (void) cctvFavoriteAdd:(NSMutableDictionary *)favoriteDic
{
    // 즐겨찾기 목록이 메모리에 없을 경우 불러오기
    if ( [OllehMapStatus sharedOllehMapStatus].favoriteList.count <= 0 ) [self getFavoriteList];
    NSLog(@"즐겨찾기 카운트 : %d", (int)[(NSMutableArray *)[OllehMapStatus sharedOllehMapStatus].favoriteList count]);

    // 기존 즐겨찾기 목록에 동일한 이름이 존재하는지 체크
    for (NSDictionary *dic in [OllehMapStatus sharedOllehMapStatus].favoriteList)
    {
        if ([[dic objectForKey:@"IconType"] intValue] == 10002 && [[dic objectForKeyGC:@"Title1"] isEqualToString:[favoriteDic objectForKeyGC:@"Title1"]])
        {
            while (![self cctvNameValidCheck:(NSMutableDictionary *)favoriteDic])
            {
                [favoriteDic setObject:[NSString stringWithFormat:@"%@_%d", [dic objectForKeyGC:@"Title1"], count] forKey:@"Title1"];
                count++;
            }
        }
    }

    NSLog(@"name : %@", [favoriteDic objectForKeyGC:@"Title1"]);
    
    [self addFavorite:favoriteDic];

}
- (BOOL) cctvNameValidCheck:(NSMutableDictionary *)favoriteDic
{
    [self initFavoriteListDB];
    ...
    for (NSDictionary *dic in [OllehMapStatus sharedOllehMapStatus].favoriteList)
    {
        if ([[favoriteDic objectForKey:@"IconType"] intValue] == 10002 && [[dic objectForKeyGC:@"Title1"] isEqualToString:[favoriteDic objectForKeyGC:@"Title1"]])
        {
            return FALSE;
        }
    }
...
- (void) addFavorite :(NSDictionary*)favoriteDic
{
    ...
    NSString *rdCd = nil;
    if( [[favoriteDic allKeys] containsObject:@"RdCd"])
        rdCd = [NSString stringWithFormat:@"%@", [favoriteDic objectForKeyGC:@"RdCd"]];
    else
        rdCd = @"";
    
    NSString *query = [NSString stringWithFormat:@"INSERT INTO FavoriteListTB (FavoriteID,SortOrder,Category,Title1,Title2,Title3,IconType,Coord1x,Coord1y,Coord2x,Coord2y,Coord3x,Coord3y,DetailType,DetailID,ShapeType,FcNm, IdBgm, RdCd)  VALUES(NULL, %@, %d, '%@', '%@', '%@', %d, %f, %f, %f, %f, %f, %f, '%@', '%@', '%@', '%@', '%@', '%@');"
                       ...
                       , coord2x, coord2y
                       , coord3x, coord3y
                       , detailType, detailID, shapeType, fcNm, idBgm, rdCd
                       ];
...
- (NSString *)GetUTF8String:(NSString *)hanggulString :(BOOL)onlyChosung
{
	...
	for (int i=0, maxi=(int)[hanggulString length] ;i < maxi;i++)
    {
