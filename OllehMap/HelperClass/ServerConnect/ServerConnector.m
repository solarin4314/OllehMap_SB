...

{
    ...
    // 쿼리 암호화
    NSString *keyStr = (NSString *)CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)[key AES128EncryptWithKey:[NSString stringWithFormat:@"K-%@-T", @"ollehmap1234"]], NULL, (CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8);
...
- (ServerRequester *) requestSearchPublicBusStation :(id)target action:(SEL)action Name:(NSString *)Name ViewCnt:(int)ViewCnt Page:(int)Page
{
    ...
    // 쿼리 암호화
    NSString *keyStr = (NSString *)CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)[Name AES128EncryptWithKey:[NSString stringWithFormat:@"K-%@-T", @"ollehmap1234"]], NULL, (CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8);
...
- (ServerRequester *) requestSearchPublicSubwayStation :(id)target action:(SEL)action Name:(NSString *)Name
{
    ...
    NSString *rawUrl = [NSString stringWithFormat:@"/v1/masstransit/TrafficSearchSubStation.json?NAME=%@", keyStr];
        ...
        @try
        {
            ...
            else if ([[request userString] isEqualToString:@"SS"])
            {
                ...
                @try
                {
                    ...
                    {
                        [oms.searchLocalDictionary setObject:[NSString stringWithFormat:@"%d",(int)arrRD.count] forKey:@"TotalCountPublicSubwayStation"];
                        [oms.searchLocalDictionary setObject:[NSString stringWithFormat:@"%d",(int)arrRD.count] forKey:@"CurrentCountPublicSubwayStation"];
...
- (void)finishPoiDetailAtPoiId:(id)request
{
	if ([request finishCode] == OMSRFinishCode_Completed)
	{
        ...
        @try
        {
            ...
            @try
            {
                if ( [[dic allKeys] containsObject:@"RESULTDATA"] && [(NSMutableDictionary *)[dic objectForKeyGC:@"RESULTDATA"] count] > 0)
                {
...
- (ServerRequester *)requestSearchURL:(id)target action:(SEL)action PX:(int)PX PY:(int)PY Query:(NSString *)query SearchType:(NSString *)searchType order:(NSString *)order
{
    ...
    NSString *keyStr = (NSString *)CFURLCreateStringByAddingPercentEscapes(NULL, (CFStringRef)query, NULL, (CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8);
    
    // 쿼리 암호화
    //NSString *keyStr = (NSString *)CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)[query AES128EncryptWithKey:[NSString stringWithFormat:@"K-%@-T", @"ollehmap1234"]], NULL, (CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8);
    
    NSString *requestLongURL = [NSString stringWithFormat:
                                @"%@ptype=m_search&x=%d&y=%d&query=%@&searchtype=%@&order=%@&coordtype=utmk",COMMON_URL_SHARE,PX, PY,keyStr, searchType, order];
...
- (ServerRequester *)requestDetailURL:(id)target action:(SEL)action PID:(NSString *)poi_id DetailType:(int)detailType Addr:(NSString *)addr StId:(NSString *)stId
{
    ...
    else
    {
        ...
        if (![poi_id isEqualToString:@""])
            [preURL appendString:[NSString stringWithFormat:@"&pid=%@", poi_id]];
    }
...
- (void) finishNoticeList:(id)request
{
    if ([request finishCode] == OMSRFinishCode_Completed)
	{
        ...
        @try
        {
            ...
            //NSLog(@"%@", [oms.noticeListDictionary objectForKeyGC:@"NOTICELIST"]);
            ...
            [oms.noticeListDictionary setObject:dic2 forKey:@"NOTICELIST"];
            //NSLog(@"%@", oms.noticeListDictionary);
            [json release];
            [dic2 autorelease];
...
- (ServerRequester *) requestThemeInfoImageDownload :(id)target action:(SEL)action downloadList:(NSArray*)downloadList downloadIndex:(NSInteger)downloadIndex
{
    ...
    [request setUserInt:(int)downloadIndex];
...
- (ServerRequester *) requestRoadNameSearch:(id)target action:(SEL)action roadCode:(NSString *)roadCode
{
    ServerRequester *request = [[[ServerRequester alloc] init] autorelease];
    
    NSString *rawUrl = [NSString stringWithFormat:@"/v2/spaceSearch/km2_sShapeSearchByRD.json?RD_CD=%@", roadCode];
    
    //NSString *refinedUrl = [self getEncodedTargetURL:rawUrl];
    
    [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@%@",COMMON_SERVER_GW, rawUrl]]];
    
    NSLog(@"도로명 검색 URL : %@", [request URL]);
    
    [request setHTTPMethod:@"GET"];
    
    [request addFinishTarget:self action:@selector(finishSearchRoadSearch:)];
...
- (void) finishSearchRoadSearch:(id)request
{
    if ([request finishCode] == OMSRFinishCode_Completed)
	{
        ...
        @try
        {
            ...
            [oms.roadNameDictionary removeAllObjects];
            
            [oms.roadNameDictionary setObject:[dic objectForKeyGC:@"RESULTDATA"] forKey:@"RoadCode"];
            //[[OllehMapStatus sharedOllehMapStatus].linePolygonArray addObjectsFromArray:[[[[[dic objectForKeyGC:@"RESULTDATA"] objectAtIndexGC:0] objectForKeyGC:@"part"] objectAtIndexGC:0] objectForKeyGC:@"vertex"]];
