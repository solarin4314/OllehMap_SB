...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {	
    ...
    NSLog(@"ED elementName = %@", elementName);
	...
	{
		NSLog(@"======> %@ :: %d ", strNBestKeys,xmlNBestData.count);
	...
	{
        if([xmlValue integerValue] > 0) {
            NSLog(@"COUNT===> %d", [xmlValue integerValue]);
            CUR_RESULT_KEY_COUNT[KEY_COUNT] = [xmlValue integerValue];
            [aResult setValue:[NSString stringWithString:xmlValue] forKey:@"COUNT"];
        } else {
            NSLog(@"COUNT NON ===> %d", [xmlValue integerValue]);
            CUR_RESULT_KEY_COUNT[KEY_COUNT] = 1;
            [aResult setValue:@"1" forKey:@"COUNT"];            
        }
	}
	else 
	{
